...
class Home {
    ...
    async dynmapbtn() {
        document.querySelector('.dynmap').addEventListener('click', async() => {
            require('electron').shell.openExternal("http://65.108.199.248:47784/")
        })}
    
    async discordbtn() {
    document.querySelector('.git').addEventListener('click', async() => {
        require('electron').shell.openExternal("https://discord.io/GochoCastle")
    })}

    async websitebtn() {
        document.querySelector('.web').addEventListener('click', async() => {
            require('electron').shell.openExternal("http://hypnostudios-colab.craftingstore.net/")
        })}
    ...
    async initStatusServer() {
        ...
        if (!serverPing.error) {
            ...
            serverMs.innerHTML = `<span class="red">Sin linea</span>`;
        }
    }
